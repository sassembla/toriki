...
- (void)startWithConsumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret
{
    ...
    self.loginURLParser = [[TKLoginURLParser alloc] initWithKey:self.consumerKey secret:self.consumerSecret];
...
{
    if (!self.isInitialized) {
        [NSException raise:@"TKInvalidInitializationException" format:@"Attempted to call TwitterKit methods before calling the requisite start method. You must call TKTwitter.sharedInstance().start(withConsumerKey:consumerSecret:) before calling any other methods."];
    }
}
...
//        アプリのURLSchemeに、TwitterAPpから起動するためのセッティングが足りない。このままだとTwitterAppから復帰できない。
        [NSException raise:@"TKInvalidInitializationException" format:@"Attempt made to Log in without a valid Twitter Kit URL Scheme set up in the app settings. Please see https://dev.twitter.com/twitterkit/ios/installation for more info."];
        return;
    }
    
    [self attemptAppLoginWithCompletion:^(
                                          NSString *username,
...
{
    BOOL isSSOBundle = [sourceApplication hasPrefix:@"com.twitter"] || [sourceApplication hasPrefix:@"com.atebits"];
    ...
    if (isSSOBundle) {
        ...
        if ([self.loginURLParser isMobileSSOSuccessURL:url]) {
            ...
            self.completion(parameters[@"username"], parameters[@"token"], parameters[@"secret"], nil);
            return;
        }
        ...
        if ([self.loginURLParser isMobileSSOCancelURL:url]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.completion(nil, nil, nil, [[NSError alloc] initWithDomain:TKLogInErrorDomain code:TKLogInErrorCodeCancelled userInfo:@{NSLocalizedDescriptionKey: @"User cancelled login from Twitter App"}]);
            });
            return;
        }
